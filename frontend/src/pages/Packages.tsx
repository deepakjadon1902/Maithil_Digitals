  ...
  return (
    <>
      ...
      <section className="bg-[#F5F8FC] px-4 py-16 sm:px-6 lg:px-8">
        <div className="fall-stagger mx-auto grid max-w-7xl auto-rows-fr gap-6 md:grid-cols-2 xl:grid-cols-3">
          {packages.map((plan) => (
            <article key={plan.name} className={`relative flex h-full min-h-[380px] flex-col rounded-premium border bg-white p-6 shadow-sm ${plan.badge ? "border-orange" : "border-navy/10"}`}>
              ...
              {plan.price ? <p className="mt-4 font-display text-3xl font-black text-orange">{plan.price}</p> : null}
              <p className="mt-4 leading-7 text-navy/65">{plan.description}</p>
              <div className="mt-5 grid gap-2 text-sm font-bold text-navy/70">
      ...
      <section className="bg-white px-4 py-16 text-navy sm:px-6 lg:px-8">
        <div className="mx-auto max-w-7xl">
          ...
          <div className="fall-stagger mt-8 grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            {packageCategories.map((category) => (
              <Link key={category.title} to={`/packages/category/${slugify(category.title)}`} target="_blank" rel="noreferrer" className="rounded-premium border border-navy/10 bg-white p-5 text-navy shadow-sm transition hover:-translate-y-1 hover:border-orange">
