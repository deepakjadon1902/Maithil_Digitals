...
export function Navbar({ settings }: { settings: SiteSettings }) {
  ...
  return (
    <header className={`fixed inset-x-0 top-0 z-40 transition ${open ? "border-b border-white/10 bg-ink/92 backdrop-blur-xl" : lightNav ? "border-b border-black/10 bg-white/94 shadow-sm backdrop-blur-xl" : "bg-transparent"}`}>
      ...
      {open ? (
        <div className="border-t border-white/10 bg-ink px-4 pb-6 pt-3 lg:hidden">
          <nav className="mx-auto grid max-w-7xl gap-1" aria-label="Mobile navigation">
            {[...nav, { label: "Contact", href: "/contact" }].map((item) => (
              <NavLink key={item.href} to={item.href} onClick={() => setOpen(false)} className="rounded-premium px-3 py-4 text-lg font-bold text-white hover:bg-white/5">
                {item.label}
              </NavLink>
            ))}
            <Button className="mt-3 w-full" href="/contact">Let's Talk</Button>
          </nav>
        </div>
      ) : null}
    </header>
